...
def construct_text_from_tokens(tokens: Iterable[Token]) -> str:
    if len(tokens) == 0: return ''
    start_pos, end_pos = tokens[0].start, tokens[-1].end
    char_list = [' '] * (end_pos - start_pos)
    for token in tokens:
        token_len = len(token.text)  # In case of non-ascii code
        i = 0
        while i < token_len:
            char_list[token.start + i - start_pos] = token.text[i]
            i += 1
...
class Annotation:
    ...
    @property
    def text(self):
        return construct_text_from_tokens(self.tokens)
...
def _read_token(row: Dict) -> Token:
    ...
    def intsplit(s: str):
        return [int(float(s)) for s in s.split('-')]
...
def _tsv_read_lines(lines: List[str], overriding_layer_names: List[Tuple[str, List[str]]] = None) -> Document:
    ...
    if tokens[0].start != 0:
        tokens.insert(0, Token(sentence_idx=0, idx=0, start=0, end=tokens[0].start, text=' '))
    sentences_start_pos = defaultdict(lambda: tokens[-1].end + 1)
    sentences_end_pos = defaultdict(lambda: 0)
    for token in tokens:
        sentences_start_pos[token.sentence_idx] = min(sentences_start_pos[token.sentence_idx], token.start)
        sentences_end_pos[token.sentence_idx] = max(sentences_end_pos[token.sentence_idx], token.end)

    # Sanity checking
    assert sorted(list(sentences_start_pos.keys())) == sorted(list(sentences_end_pos.keys())), "Bug in construt webanno sentences"

    # Reconstruct text from token text
    text = construct_text_from_tokens(tokens)
    sentences = []
    for sentence_idx in sentences_start_pos.keys():
        start_pos, end_pos = sentences_start_pos[sentence_idx], sentences_end_pos[sentence_idx]
